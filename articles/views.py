...
class ArticleList(generics.ListCreateAPIView):
    queryset = Article.objects.prefetch_related("author", "article_favorites")
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            serializer = self.get_serializer(
                                                page, many=True, 
                                                context={
                                                    "user_following": self.get_user_following(),
                                                    "article_favorites": self.get_article_favorites()
                                            })
...
class ArticleDetail(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(  
                                            self.get_object(), data=request.data.get("article", {}), 
                                            partial=partial, 
                                            context={
                                                "user_following": self.get_user_following(),
                                                "article_favorites": self.get_article_favorites(),
